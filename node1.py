...
def client(Port, RightPort, LeftPort):
	...
	clientSock.sendto("hi from " + str(Port), ("127.0.0.5", RightPort))
	clientSock.sendto("hi from " + str(Port), ("127.0.0.6", LeftPort))



def main():
	# creating thread 
    t1 = threading.Thread(target=server, args=("127.0.0.42", 5000)) 
    t2 = threading.Thread(target=client, args=(5000, 5050, 5060)) #right left
    # starting thread 1 
    t1.start() 
    # starting thread 2
    time.sleep(5) 
